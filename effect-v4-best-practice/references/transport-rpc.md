...
**v4 change**: `Schema.Union(A, B)` → `Schema.Union([A, B])` (variadic to array).
...
import { AccountRpcGroup } from "../rpc/schema.ts";
...
**v4 note**: Import `ServiceMap` from `effect` for service definitions.
...
const RpcLayer = RpcServer.layer(AllRpcGroup).pipe(Layer.provide(RpcServerHandlers), Layer.provide(ServiceLayer));
```

**v4 note**: Layers are memoized across `Effect.provide` calls by default. Prefer composing layers before providing.
